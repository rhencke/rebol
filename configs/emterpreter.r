REBOL [
    ...
    Description: {
        ...
        the  worker cannot receive data from the GUI.  The "Emterpreter" works
        around this limitation by running a JavaScript bytecode simulator...
        so even if a JavaScript stack can't pause, the bytecode interpreter
        can--at least long enough to release the GUI thread it was running on
        to do let it do work:
        ...
        and much more than twice as slow.  It is supplanted entirely with a
        superior approach based on WASM threads.  In that model, the GUI
        thread is left free, while the code that's going to make demands runs
        on its own thread...which can suspend and wait, using conventional
        atomics (mutexes, wait conditions).
        ...
        thread-based solution will be available.  The emterpreter method is
